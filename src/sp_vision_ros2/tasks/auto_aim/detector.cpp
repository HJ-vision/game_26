...
std::list<Armor> Detector::detect(const cv::Mat & bgr_img, int frame_count)
{
  ...
  // [FIX] 实机模式禁止 cv::imshow：ROS2 无 GUI 事件循环，imshow 会触发阻塞式刷新，
  // Jetson 上单帧浪费 10-30ms，直接把帧率压到 18Hz。调试画面走 /aim_debug_image。
  // cv::imshow("binary_img", binary_img);

  // 获取轮廓点
  std::vector<std::vector<cv::Point>> contours;
  // [FIX] CHAIN_APPROX_NONE 保存每个像素点，轮廓点数量是实际拐点的 10~50 倍，
  // 后续 minAreaRect、颜色遍历、几何判断全部被放大。改用 CHAIN_APPROX_SIMPLE，
  // 只保留水平/垂直/对角段端点，灯条矩形拟合不受影响。
  ...
  // [DIAG] 漏斗计数：定位"间歇性检测为空"是哪一级过滤导致
  int n_pair = 0, n_armor_geo_fail = 0, n_name_fail = 0, n_type_fail = 0;
  double last_conf = -1.0;
  ...
  // [DIAG] 最终为空但存在候选时，打印漏斗，看是哪一级杀掉了检测
  // debug 级：分类持续抖动时每帧都会发，只写文件
  if (armors.empty() && n_pair > 0) {
    tools::logger()->debug(
      "[Detector] empty! pairs={} geo_fail={} name_fail={} type_fail={} last_conf={:.3f}",
      n_pair, n_armor_geo_fail, n_name_fail, n_type_fail, last_conf);
  }
...
bool Detector::detect(Armor & armor, const cv::Mat & bgr_img)
{
  ...
  cv::findContours(binary_img, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
  std::list<Lightbar> lightbars;
  std::size_t lightbar_id = 0;  // [FIX] 本重载函数内独立计数，与主 detect 的声明保持一致
  for (const auto & contour : contours) {
    auto rotated_rect = cv::minAreaRect(contour);
    auto lightbar = Lightbar(rotated_rect, lightbar_id);
...
{
  // [FIX] 原实现遍历 contour 所有像素，CHAIN_APPROX_SIMPLE 后只剩拐点，
  // 点数太少且角上容易踩背景。改用旋转矩形包围盒内 BGR 通道均值比色，
  // 采样点固定（~20个），耗时稳定且判断更抗噪。
  auto bb = cv::boundingRect(contour);
  if (bb.area() <= 1) return Color::blue;  // 异常兜底
  // 采样 5x5=25 点（步长覆盖整个bb），避开背景边缘（留 1px）
  constexpr int N = 5;
  int red_sum = 0, blue_sum = 0, count = 0;
  for (int i = 0; i < N; ++i) {
    int y = bb.y + 1 + i * (bb.height - 2) / (N - 1);
    if (y < 0 || y >= bgr_img.rows) continue;
    for (int j = 0; j < N; ++j) {
      int x = bb.x + 1 + j * (bb.width - 2) / (N - 1);
      if (x < 0 || x >= bgr_img.cols) continue;
      const auto & px = bgr_img.at<cv::Vec3b>(y, x);
      red_sum += px[2];
      blue_sum += px[0];
      ++count;
    }
  }
  if (count < 4) return Color::blue;
