...
: debug_(debug), detector_(config_path, false)
{
  ...
  // [FIX] 输入分辨率可配置：旧配置缺省时兜底 640
  input_size_ = yaml["input_size"] ? yaml["input_size"].as<int>() : 640;
  if (input_size_ % 32 != 0) {
    tools::logger()->warn("[YOLO11] input_size={} 不是 32 的倍数，向上取整", input_size_);
  ...
  try {
    ...
    tools::logger()->warn("[YOLO11] FP16 compile failed ({}), fallback to LATENCY-only",
                          e.what());
