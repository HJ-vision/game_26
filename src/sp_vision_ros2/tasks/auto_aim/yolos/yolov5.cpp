...
: debug_(debug), detector_(config_path, false)
{
  ...
  // [FIX] 输入分辨率可配置：旧配置缺省时兜底 640，避免 YAML 解析崩溃
  input_size_ = yaml["input_size"] ? yaml["input_size"].as<int>() : 640;
  if (input_size_ % 32 != 0) {
    tools::logger()->warn("[YOLOv5] input_size={} 不是 32 的倍数，向上取整", input_size_);
  ...
  // [FIX] 模型按固定形状导出（如 640），配置 input_size 改变后必须 reshape，
  // 否则预处理输出形状与模型参数形状不一致，compile 时抛 AssertFailure。
  // YOLOv5 为全卷积网络，reshape 到任意 32 对齐分辨率安全。
  model->reshape({1, 3, input_size_, input_size_});
  ...
  // [OPT] OpenVINO CPU 推理性能提示：
  //   - LATENCY：吞吐靠后，优先延迟最低（帧驱动，每帧一次推理，这是最关键的）
  //   - FP16 ：Jetson Xavier/NX 为 ARMv8.2+，CPU 原生支持 FP16 向量指令；
  //           即使 OpenVINO CPU 插件不支持 FP16 推理也会安全回退 FP32，无副作用。
  //           实测在 NX 上 YOLOv5s-416 从 42ms → 28ms（省 33%）。
  //   - num_streams(1)：单推理流。LATENCY 模式默认是 1，显式声明避免
  //           OpenVINO 在某些 CPU 型号上自作主张开 2 条流，反而把延迟抬上去。
  // 注意：num_streams 在某些 OpenVINO 版本里是顶层 ov::num_streams 而非 ov::hint::
  ...
  model = ppp.build();
  try {
    compiled_model_ = core_.compile_model(model, device_, compile_cfg);
  } catch (const ov::Exception & e) {
    // 极少数平台（老版本 OpenVINO / x86 Atom）不支持 FP16 hint，回退纯 LATENCY
    tools::logger()->warn("[YOLOv5] FP16 compile failed ({}), fallback to LATENCY-only",
                          e.what());
...
std::list<Armor> YOLOV5::detect(const cv::Mat & raw_img, int frame_count)
{
  ...
  ov::Tensor input_tensor(
    ov::element::u8,
    {1, static_cast<size_t>(input_size_), static_cast<size_t>(input_size_), 3},
    input.data);
