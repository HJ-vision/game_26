...
: classifier_(config_path), detector_(config_path), debug_(debug)
{
  ...
  // [FIX] 输入分辨率可配置：旧配置缺省时兜底 416（yolov8 默认就是 416）
  input_size_ = yaml["input_size"] ? yaml["input_size"].as<int>() : 416;
  if (input_size_ % 32 != 0) {
    tools::logger()->warn("[YOLOv8] input_size={} 不是 32 的倍数，向上取整", input_size_);
  ...
  // [FIX] 模型按固定形状导出，配置 input_size 改变后必须 reshape，
  ...
  // [OPT] OpenVINO CPU 推理性能提示（同 yolov5.cpp 说明）
  ...
  try {
    ...
    tools::logger()->warn("[YOLOv8] FP16 compile failed ({}), fallback to LATENCY-only",
                          e.what());
